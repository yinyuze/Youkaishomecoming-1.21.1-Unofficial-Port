...
public class KoishiSpell extends ActualSpellCard {
	...
	private void homing(CardHolder holder, double dist) {
		...
		if (tick % 10 == 0) {
			...
			for (int i = 0; i < n; i++) {
				...
				var e = holder.prepareDanmaku(40, dir, DanmakuItems.Bullet.BALL, DyeColor.RED);
				holder.shoot(e);
			}
		}
	}
	...
	private void addLaser(CardHolder holder, Vec3 target, double t) {
		...
		var e = holder.prepareLaser(40, pos, dir, 60, DanmakuItems.Laser.LASER, DyeColor.BLUE);
	...
	private void border(CardHolder holder, Vec3 target, Vec3 vel) {
		...
		for (int i = 0; i < n; i++) {
			...
			var e = holder.prepareDanmaku(40, forward, DanmakuItems.Bullet.CIRCLE, DyeColor.PINK);
			e.setPos(pos);
			holder.shoot(e);
		}
	}
	...
	public static class StateChange extends Ticker<KoishiSpell> {
		...
		private void step(CardHolder holder) {
			...
			if (tick == 0) {
				...
				var e = holder.prepareDanmaku(t0, vec, DanmakuItems.Bullet.MENTOS, DyeColor.RED);
			...
			if (tick == t0) {
				...
				var e = holder.prepareDanmaku(t, vec, DanmakuItems.Bullet.MENTOS, DyeColor.RED);
				e.setPos(p0);
