...
public class DoremiSpell extends ActualSpellCard {
	...
	public void tick(CardHolder holder) {
		...
		if (holder.self() instanceof Mob e) {
			var le = e.getTarget();
			if (le != null) {
				...
				for (int i = 0; i < n; i++) {
					...
					for (int j = 0; j < m; j++) {
						...
						var l = holder.prepareLaser(120, pi, dir, 40,
								DanmakuItems.Laser.LASER, DyeColor.RED);
		...
		private void add(CardHolder holder, Vec3 pos, int step, int n, double dist, double tilt) {
			...
			for (int i = 0; i < n; i++) {
				...
				var e = holder.prepareDanmaku(life, v,
						DanmakuItems.Bullet.BALL, i % 2 == 0 ? DyeColor.YELLOW : DyeColor.ORANGE);
		...
		private void chase(CardHolder holder) {
			...
			if (target != null && tick % 4 == 0 && Math.abs(target.y() - pos.y() - 0.3) > 2) {
				var e = holder.prepareDanmaku(2,
						...
						DanmakuItems.Bullet.BALL, DyeColor.RED);
			...
			for (int i = 0; i < n; i++) {
				...
				for (int j = 0; j < m; j++) {
					...
					var e = holder.prepareDanmaku(100, dir.scale(v),
							DanmakuItems.Bullet.BALL, j % 2 == 0 ? DyeColor.BLUE : DyeColor.MAGENTA);
					e.setPos(p);
					holder.shoot(e);
				}
			}
