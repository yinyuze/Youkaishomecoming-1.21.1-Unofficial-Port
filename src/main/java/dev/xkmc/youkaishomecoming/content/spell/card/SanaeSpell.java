...
public class SanaeSpell extends ActualSpellCard {
	...
	private void near(CardHolder holder, Vec3 le) {
		...
		if (tick % 10 == 0) {
			...
			holder.shoot(holder.prepareDanmaku(80, dir, DanmakuItems.Bullet.CIRCLE, DyeColor.RED));
		}
		if (tick % 40 == 0) {
	...
	public static class RotatingStar extends Ticker<SanaeSpell> {
		...
		private void step(CardHolder holder) {
			...
			var e = holder.prepareLaser(40, pos, dir, 40, DanmakuItems.Laser.LASER, DyeColor.LIGHT_BLUE);
			...
			if (tick < 10 && tick % 2 == 0) {
				...
				for (int i = 0; i < 5; i++) {
					...
					for (int j = 0; j < 5; j++) {
						...
						var e = holder.prepareDanmaku(dur, d1, DanmakuItems.Bullet.CIRCLE, DyeColor.RED);
						e.setPos(p0);
						holder.shoot(e);
					}
					var d1 = d0.scale(speed);
					var e = holder.prepareDanmaku(60, d1, DanmakuItems.Bullet.CIRCLE, DyeColor.RED);
					e.setPos(p0);
					holder.shoot(e);
				}
			}
				...
				for (int i = 0; i < 5; i++) {
					...
					for (int j = 0; j < 5; j++) {
						...
						for (int k = 0; k < 3; k++) {
							...
							var e = holder.prepareDanmaku(life, d2, DanmakuItems.Bullet.BALL, COLORS[j]);
							e.setPos(p1);
							holder.shoot(e);
						}
					}
