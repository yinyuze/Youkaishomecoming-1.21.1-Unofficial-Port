...
public class ClientModelManager extends ModelHolderManager {
	...
	private void onAdditionalModel(ModelEvent.RegisterAdditional event) {
		var index = GensokyoLegacy.loc("additional_models.json");
		...
		for (Resource file : files) {
			try (Reader reader = new BufferedReader(new InputStreamReader(file.open(), StandardCharsets.UTF_8))) {
				...
				GensokyoLegacy.LOGGER.error("Couldn't read global additional model list {} in data pack {}", index, file.sourcePackId(), e);
			}
		}
		for (var e : list) {
			event.register(net.minecraft.client.resources.model.ModelResourceLocation.standalone(e));
		}
	}
