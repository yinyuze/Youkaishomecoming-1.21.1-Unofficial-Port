...
public abstract class BasePotScreen<T extends BasePotMenu<R>, R extends BasePotRecipe> extends AbstractContainerScreen<T> implements RecipeUpdateListener {
	...
	protected void containerTick() {
		super.containerTick();
		if (!bookFailed) book.tick();
	}

	public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTicks) {
		if (!bookFailed && book.isVisible() && widthTooNarrow) {
		...
		if (!bookFailed) book.renderTooltip(gui, leftPos, topPos, mouseX, mouseY);
	}
	...
	protected boolean isHovering(int x, int y, int width, int height, double mouseX, double mouseY) {
		return (bookFailed || !widthTooNarrow || !book.isVisible()) && super.isHovering(x, y, width, height, mouseX, mouseY);
	}

	public boolean mouseClicked(double mouseX, double mouseY, int buttonId) {
		if (!bookFailed && book.mouseClicked(mouseX, mouseY, buttonId)) {
			setFocused(book);
			return true;
		} else {
			return !bookFailed && widthTooNarrow && book.isVisible() || super.mouseClicked(mouseX, mouseY, buttonId);
		}
	}
	...
	protected boolean hasClickedOutside(double mouseX, double mouseY, int x, int y, int buttonIdx) {
		...
		return flag && (bookFailed || book.hasClickedOutside(mouseX, mouseY, leftPos, topPos, imageWidth, imageHeight, buttonIdx));
	}

	protected void slotClicked(Slot slot, int mouseX, int mouseY, ClickType clickType) {
		super.slotClicked(slot, mouseX, mouseY, clickType);
		if (!bookFailed) book.slotClicked(slot);
	}

	public void recipesUpdated() {
		if (!bookFailed) book.recipesUpdated();
	}
