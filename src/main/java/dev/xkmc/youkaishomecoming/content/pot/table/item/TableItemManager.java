...
// import vectorwing.farmersdelight.common.tag.ForgeTags; // Not available in 1.21.1
...
public class TableItemManager extends BaseTableItem {
	...
	public static final ModelHolderManager MANAGER = ModelHolderManager.createModelBuilderManager(GensokyoLegacy.REGISTRATE);

	private static FixedModelHolder fixed(String id) {
		return new FixedModelHolder(MANAGER, GensokyoLegacy.loc("fixed/" + id));
	}

	private static VariantModelHolder variant(String id) {
		return new VariantModelHolder(MANAGER, GensokyoLegacy.loc(id));
	}

	private static AdditionalModelHolder top(String id) {
		return new AdditionalModelHolder(MANAGER, GensokyoLegacy.loc(id));
	}
	...
	public static final VariantTableItemBase BASE_SUSHI = SUSHI.asBase(GensokyoLegacy.loc("sushi"));
	public static final VariantTableItemBase BASE_GUNKAN = GUNKAN.asBase(GensokyoLegacy.loc("gunkan"));
	public static final VariantTableItemBase BASE_HOSOMAKI = HOSOMAKI.asBase(GensokyoLegacy.loc("hosomaki"));
	public static final VariantTableItemBase BASE_FUTOMAKI = FUTOMAKI.asBase(GensokyoLegacy.loc("futomaki"));
	public static final VariantTableItemBase BASE_CAL = CAL.asBase(GensokyoLegacy.loc("california"));
	...
	private static void addBulk(String id, String path, ItemLike item, VariantModelPart... parts) {
		for (var part : parts) {
			part.addMapping(id, item).tex(GensokyoLegacy.loc("block/table/" + path));
		}
	}
