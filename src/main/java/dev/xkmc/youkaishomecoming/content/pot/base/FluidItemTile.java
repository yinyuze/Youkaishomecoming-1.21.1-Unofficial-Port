...
public interface FluidItemTile {
	...
	static ItemInteractionResult addFluidOrItem(FluidItemTile be, ItemStack stack, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
		...
		if (!hasFluid || stack.getItem() instanceof SlipBottleItem || stack.getItem() instanceof SakeBottleItem) {
			var opt = stack.getCapability(Capabilities.FluidHandler.ITEM);
			if (opt != null) {
		...
		var potionContents = stack.getOrDefault(net.minecraft.core.component.DataComponents.POTION_CONTENTS, PotionContents.EMPTY);
		if (stack.is(Items.POTION) && potionContents.potion().isPresent() && potionContents.potion().get().is(Potions.WATER)) {
