...
public class TileInfoDisplay implements LayeredDraw.Layer {
	...
	public static class ImageBox extends OverlayUtil {
		...
		public void render(InfoTile tile) {
			...
			var hit = Minecraft.getInstance().hitResult;
			if (!(hit instanceof BlockHitResult bhit)) return;
			boolean shift = Minecraft.getInstance().player != null && Minecraft.getInstance().player.isShiftKeyDown();
			List<ClientTooltipComponent> tooltip = new ArrayList<>();
			var image = tile.getImage(shift, bhit);
			if (image != null) {
				tooltip.add(new TileClientTooltip(image));
			}
			for (var e : tile.lines(shift, bhit)) {
