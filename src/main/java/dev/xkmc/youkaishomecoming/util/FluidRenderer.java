...
public class FluidRenderer {
	...
	public static void renderTextureBox(ResourceLocation texture, float xMin, float yMin, float zMin, float xMax, float yMax, float zMax, MultiBufferSource buffer, PoseStack ms, int light, boolean renderBottom, int color) {
		renderTextureBox(texture, xMin, yMin, zMin, xMax, yMax, zMax, getFluidBuilder(buffer), ms, light, renderBottom, color);
	}

	public static void renderTextureBox(ResourceLocation texture, float xMin, float yMin, float zMin, float xMax, float yMax, float zMax, VertexConsumer builder, PoseStack ms, int light, boolean renderBottom, int color) {
			...
			if (side != Direction.DOWN || renderBottom) {
				...
				if (side.getAxis().isHorizontal()) {
					if (side.getAxis() == Axis.X) {
						renderStillTiledFace(side, zMin, yMin, zMax, yMax, positive ? xMax : xMin, builder, ms, light, color, sprite);
					} else {
						renderStillTiledFace(side, xMin, yMin, xMax, yMax, positive ? zMax : zMin, builder, ms, light, color, sprite);
					}
				} else {
					renderStillTiledFace(side, xMin, zMin, xMax, zMax, positive ? yMax : yMin, builder, ms, light, color, sprite);
				}
			}
		}
	...
	public static void renderFluidBox(FluidStack fluidStack, float xMin, float yMin, float zMin, float xMax, float yMax, float zMax, VertexConsumer builder, PoseStack ms, int light, boolean renderBottom, int colorOverride) {
		...
		if (stillTexture == null) {
			dev.xkmc.youkaishomecoming.init.GensokyoLegacy.LOGGER.error("Fluid {} has null still texture", BuiltInRegistries.FLUID.getKey(fluid));
			return;
		}
		TextureAtlasSprite fluidTexture = Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(stillTexture);
	...
	private static class RenderTypes extends RenderStateShard {
		...
		private static String createLayerName(String name) {
			return GensokyoLegacy.MODID + ":" + name;
		}
