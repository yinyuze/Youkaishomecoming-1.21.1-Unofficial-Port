...
public enum YHSake implements IYHSake {
	...
	YHSake(FoodType type, int color, List<EffectEntry> effs, TagKey<Item>... tags) {
		...
		fluid = GensokyoLegacy.REGISTRATE.entry(name, (c) -> new VirtualFluidBuilder<>(
						GensokyoLegacy.REGISTRATE, GensokyoLegacy.REGISTRATE,
						name, c,
						ResourceLocation.withDefaultNamespace("block/water_still"),
						ResourceLocation.withDefaultNamespace("block/water_flow"),
						(p, s, f) -> new SakeFluidType(p, s, f, this),
						p -> new SakeFluid(p, this)))
				.defaultLang().register();
		item = type.build(GensokyoLegacy.REGISTRATE,
				p -> new SakeBottleItem(fluid::getSource, p), "sake/", name, 0, 0, tags, effs);
	}
